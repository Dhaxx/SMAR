...
def depreciacoes():
    ...
    consulta = fetchallmap('''
                                select
                                    case
                                        when tipatr = 'M' then 1
                                        when tipatr = 'V' then 2
                                        when tipatr = 'I' then 3
                                        when tipatr = 'A' then 4 end as grupo,
                                    t808.idpatrimonio,
                                        t808.dtcalculo,
                                        'R' tipo_mov,
    ...
    cadpat = cur_fdb.execute('select codigo_gru_pat, codigo_ant_pat, codigo_pat from pt_cadpat').fetchall()
    codigos = {(codigo_gru_pat, codigo_ant): codigo for codigo_gru_pat, codigo_ant, codigo in cadpat}

    for row in tqdm(consulta, desc='PATRIMÔNIO - DEPRECIAÇÕES'):
        try:
            codigo_mov += 1
            empresa_mov = EMPRESA
            codigo_pat_mov = codigos[(row['grupo'], row['idpatrimonio'])]
...
def transferencias():
    ...
    cria_campo('alter table pt_movbem add origem_smar varchar(19)')
    cria_campo('alter table pt_movbem add destino_smar varchar(19)')
    ...
    aux2 = cur_fdb.execute('select codigo_pat, codigo_ant_pat from pt_cadpat').fetchallmap()
    codigos = {}
    for row in aux2:
        codigos[row['codigo_ant_pat']] = row['codigo_pat']
    ...
    for row in tqdm(consulta, desc='Inserindo Transferências:'):
        codigo_mov += 1
        empresa_mov = EMPRESA
        codigo_pat_mov = codigos.get(row['codigo_pat_mov'], 0)
        data_mov = row['cmpdta']
        tipo_mov = 'T'
        codigo_cpl_mov = None
        ...
        destino_smar = row['destino']
        origem_smar = row['origem']
        ...
        cur_fdb.execute(insert, (codigo_mov, empresa_mov, codigo_pat_mov, data_mov, tipo_mov, codigo_cpl_mov, codigo_set_mov, valor_mov, historico_mov,
                                 lote_mov, percentual_mov, depreciacao_mov, codigo_set_ant, destino_smar, origem_smar))
    commit()